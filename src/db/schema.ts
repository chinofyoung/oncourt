import { pgTable, index, foreignKey, unique, uuid, text, timestamp, integer, check, boolean, jsonb, date, uniqueIndex, smallint, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const bookingStatus = pgEnum("booking_status", ['pending_payment', 'confirmed', 'completed', 'expired', 'refunded_manual', 'blocked', 'pending_verification'])
...
export const emailKind = pgEnum("email_kind", ['booking_confirmed', 'booking_new', 'booking_reminder', 'court_moderated', 'refund_recorded', 'manual_proof_submitted', 'manual_proof_rejected', 'manual_review_expired'])
export const emailStatus = pgEnum("email_status", ['pending', 'sent', 'failed'])
export const manualProofStatus = pgEnum("manual_proof_status", ['pending', 'approved', 'rejected'])
export const paymentMethodKind = pgEnum("payment_method_kind", ['bank', 'ewallet'])
export const paymentMode = pgEnum("payment_mode", ['automated', 'manual'])
...
export const profiles = pgTable("profiles", {
	...
	paymentMode: paymentMode("payment_mode").default('automated').notNull(),
	manualReviewMinutes: integer("manual_review_minutes"),
}, (table) => [
	foreignKey({
			columns: [table.id],
	...
	check("profiles_manual_review_window", sql`(manual_review_minutes IS NULL) OR ((manual_review_minutes >= 30) AND (manual_review_minutes <= 10080))`),
]);
...
export const ownerPaymentMethods = pgTable("owner_payment_methods", {
	...
	index("owner_payment_methods_owner_id_idx").using("btree", table.ownerId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.ownerId],
			foreignColumns: [profiles.id],
			name: "owner_payment_methods_owner_id_fkey"
		}).onDelete("cascade"),
]);
...
export const bookings = pgTable("bookings", {
	...
	index("bookings_court_starts_at_idx").using("btree", table.courtId.asc().nullsLast().op("timestamptz_ops"), table.startsAt.asc().nullsLast().op("uuid_ops")),
	...
	index("bookings_payment_mode_idx").using("btree", table.paymentMode.asc().nullsLast().op("enum_ops")),
...
export const manualPaymentProofs = pgTable("manual_payment_proofs", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	bookingId: uuid("booking_id").notNull(),
	ownerPaymentMethodId: uuid("owner_payment_method_id"),
	paidToSnapshot: jsonb("paid_to_snapshot").notNull(),
	storagePath: text("storage_path").notNull(),
	referenceNote: text("reference_note"),
	status: manualProofStatus().default('pending').notNull(),
	submittedAt: timestamp("submitted_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	reviewedAt: timestamp("reviewed_at", { withTimezone: true, mode: 'string' }),
	reviewedBy: uuid("reviewed_by"),
	rejectionReason: text("rejection_reason"),
}, (table) => [
	index("manual_payment_proofs_booking_id_idx").using("btree", table.bookingId.asc().nullsLast().op("uuid_ops")),
	index("manual_payment_proofs_method_idx").using("btree", table.ownerPaymentMethodId.asc().nullsLast().op("uuid_ops")),
	index("manual_payment_proofs_pending_idx").using("btree", table.status.asc().nullsLast().op("enum_ops")).where(sql`(status = 'pending'::manual_proof_status)`),
	index("manual_payment_proofs_reviewed_by_idx").using("btree", table.reviewedBy.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.bookingId],
			foreignColumns: [bookings.id],
			name: "manual_payment_proofs_booking_id_fkey"
		}),
	foreignKey({
			columns: [table.ownerPaymentMethodId],
			foreignColumns: [ownerPaymentMethods.id],
			name: "manual_payment_proofs_owner_payment_method_id_fkey"
		}).onDelete("set null"),
	foreignKey({
			columns: [table.reviewedBy],
			foreignColumns: [profiles.id],
			name: "manual_payment_proofs_reviewed_by_fkey"
		}),
	unique("manual_payment_proofs_booking_id_key").on(table.bookingId),
	check("manual_proof_rejection_reason", sql`(status = 'rejected'::manual_proof_status) = (rejection_reason IS NOT NULL)`),
	check("manual_proof_reviewed_pair", sql`(status = 'pending'::manual_proof_status) = (reviewed_at IS NULL)`),
]);
...
export const emailOutbox = pgTable("email_outbox", {
	...
	uniqueIndex("email_outbox_booking_kind_idx").using("btree", table.kind.asc().nullsLast().op("uuid_ops"), table.bookingId.asc().nullsLast().op("enum_ops")).where(sql`((booking_id IS NOT NULL) AND (kind <> 'refund_recorded'::email_kind))`),
...
export const platformSettings = pgTable("platform_settings", {
	...
	defaultManualReviewMinutes: integer("default_manual_review_minutes").default(1440).notNull(),
}, (table) => [
	...
	check("platform_settings_manual_review_window", sql`(default_manual_review_minutes >= 30) AND (default_manual_review_minutes <= 10080)`),
