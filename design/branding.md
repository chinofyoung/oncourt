...
- Focus: `outline: 2px solid var(--ball); outline-offset: 3px` on dark, `var(--court)` on light. **Never pair the focus utilities with `outline-none` (or `outline-hidden`) on the same element** — in Tailwind v4 that is not a no-op, it silently kills the ring. `outline-none` compiles to an *ungated* `--tw-outline-style: none`, and `focus-visible:outline-2` renders `outline-style: var(--tw-outline-style)`, so the pinned `none` wins even while `:focus-visible` matches. The element then computes `outline-style: none` when focused and shows nothing, while the source reads as if it has a focus ring. Just omit the base utility: the `@property` registration for `--tw-outline-style` declares `initial-value: solid`, so `focus-visible:outline-*` resolves correctly on its own. This bit both search bars — the hero's four controls had no visible ring at all and the float's classes never rendered one despite looking right — and was only caught by tabbing to each control and reading `getComputedStyle(el).outlineStyle`. Verify a focus ring with real keyboard focus; `.focus()` from the console does not reliably match `:focus-visible`.
  **Documented exception — the venue page's photo gallery tiles** (`src/components/branch/photo-gallery.tsx`): the gallery's grid tiles use `focus-visible:outline-offset-[-3px]` — inset, not the `3px` outward offset above. The tiles sit edge-to-edge in a 2px grid gap, so an outward ring on one tile would run into its neighbor (or get clipped by the grid's own `overflow-hidden`); drawing the ring inward instead keeps it fully visible without widening the gap. Everywhere else in the app, the outward `3px` offset above is still the rule — this is scoped to grid tiles packed this tightly, not a general alternative.
...
- **Time-range field:** a span is ONE field holding TWO selects with the word "to" between them (`[ Any time ▾ ] to [ — ▾ ]`), not two fields and not two grid columns — the label sits above the pair, and the connector is `aria-hidden` decoration. This was a spaced en dash (`[ Any time ▾ ] – [ — ▾ ]`) until 2026-08-15, matching the `7 – 9 AM` convention under "Currency & data formatting" — reversed because that convention is for a *rendered* range, and between two live dropdowns a dash reads as a hyphen inside a value rather than as a connector; see that section's note for the distinction. The empty start option reads "Any time"; the empty end option is an em dash, meaning "just that hour". Each select needs its own accessible name ("Time from" / "Time until"), since one visible label can't name two controls. The end select narrows to hours after the chosen start and goes inert with no start — but only where the page is already a client component; a server-rendered GET form lists all end hours and lets the server drop an impossible pair, so client narrowing is never load-bearing. Both selects need `min-w-0` (and their container too): the `truncate` on a select refuses to shrink below its text width, and two of them in one cell will otherwise push the page sideways, which the Layout rule forbids. Below 560px give the field the full row rather than making two selects share half of a phone screen. Built twice, on purpose: `src/app/page.tsx`'s hero (white-on-glass) and `src/components/search/map-hero.tsx`'s float (ink-on-panel) share the hour logic in `src/lib/search/hours.ts` but not the markup — the two skins have nothing stylable in common. What they DO share, as of 2026-08-07: the same 1120px content column, the same `grid-cols-[1.25fr_1fr_1fr_auto]` track list (the float was previously a shrink-to-fit flex row ~⅔ the width, which squeezed each hour select to 73px and visibly clipped "Any time" to "Any t…"), the same `p-2`/`gap-2`/56px-field metrics, the same 15.5px field text, and the same chevron. The outer height still differs by 2px — float 72px, hero 74px — because the home hero's glass skin adds a `border border-white/[.18]` that the float's solid `--panel` card correctly omits; that 2px is a property of the skin, not drift to fix. As of 2026-08-15, all three field cells in both skins (Where, Date, Time) also carry a persistent resting fill instead of the previous hover-only wash — a cell showed no background at all until hovered, which gave no resting signal that it was an interactive dropdown. The hero now sits at `bg-white/[.07]` at rest, brightening to `hover:bg-white/[.12]`; the float sits at `--surface` at rest, brightening to `hover:bg-[var(--band-off)]` (chosen over `--booked` — see Color table — because `--booked` is reserved for neutral/disabled tags, not an interactive hover, per its own comment in `globals.css`). The `border-l` divider that used to separate cells was dropped in the same pass: once every cell reads as its own filled, rounded panel, a line cutting between them is leftover chrome from when the cells were transparent. Keep new values matched across the two files rather than extracting a shared component — the drift is what needs fixing, not the duplication.
...
- **Nav:** floating over heroes (absolute, transparent, white text + glass pill) or solid `--surface` with hairline border on utility pages. Right side, signed in: a role link (e.g. "Owner dashboard", "Admin", "My bookings") directly beside the 36px avatar / account menu; signed out: a plain "Sign in" link. The role link is the *first* entry of `accountLinks()` (`src/components/site/account-menu.tsx`) — the same ordered, role-derived list the account menu's dropdown renders in full — so the nav's one-click shortcut and the dropdown can never disagree about which destination leads: promoting an item to the nav is a rendering choice about the first entry, not a second copy of the precedence rule. It hides at ≤980px, the same breakpoint that hides "Find courts"/"Top rated" (see Breakpoints) — the account menu's dropdown still carries every destination at that width, so nothing is lost, only the one-click shortcut. The "List your court" pill that used to sit beside the avatar was removed from the header on 2026-08-08 — the home hero and footer already carry that CTA (`ownerCtaHref()`, `src/lib/site/owner-cta.ts`), so the nav's copy was redundant on every page; those two CTAs still stand.
...
- **Rating:** five stars (`src/components/ui/stars.tsx`), filled to the nearest half — a muted `--hairline` row of five stars with a clipped `--court` overlay on top; a half-star is whatever fraction of the overlay's width the clip reveals, not a separate glyph, so the same code renders any fraction if the rounding rule ever changes. Filled `--court`, empty `--hairline` — deliberately not `--ball`: this doc's Color section reserves lime as THE accent, "use sparingly," and the mark this replaces was a single 7px dot; five lime glyphs on every card in a search grid is the opposite of sparing, and lime on white needs an ink outline to stay legible at all, which is why that dot had one and stars don't. The numeric average still sits beside the stars (bold, e.g. `4.3`) rather than being replaced by them, because half-star rounding makes 4.3 and 4.7 look identical — the stars are the fast read, the number is the true one; count stays in parens, muted. Renders nothing at zero reviews (`src/components/ui/rating.tsx`) — a "0 stars (0)" row reads as a bad rating, not an absent one. The star row itself is `aria-hidden`; the caller supplies the accessible name (`role="img"` + an `aria-label` naming the average and count, e.g. "Rated 4.3 out of 5 from 12 reviews", or "You rated this 5 out of 5" on a single review), so nothing announces itself twice. `src/components/branch/review-list.tsx`'s per-review rating row composes this same `Stars` primitive (`size={12}`, wrapped in `role="img"` + `aria-label`), so the branch page shows one consistent star mark throughout rather than a star aggregate above circle per-review marks.
- **Rating input:** five radios in a `<fieldset>` styled as stars, never click handlers on spans — radios are arrow-key navigable, announced as a group, and submit with no JavaScript at all (`src/app/bookings/review-form.tsx`; no `useState`). The five `<label>`s sit in natural 1–5 DOM order — no reversal, no `flex-row-reverse` — with the row right-aligned via `justify-end` instead. Each label fills on its own radio's `:checked` (`has-[:checked]`) and ALSO fills whenever any later sibling label's radio is checked or hovered (`:has(~ label input:checked)`, `:has(~ label:hover)`); since "later" in DOM order means "higher rating" here, checking star 5 lights all five and checking star 3 lights 1–3 — full-looking fill with zero JS. An earlier draft got the same visual goal backwards: reversed DOM order paired with `flex-row-reverse`, which inverted the fill instead of producing it. That version isn't what's built — natural order plus forward-looking `:has()` is the only one that's correct, so don't reintroduce the reversal reading this cold.
- **Modal:** a native `<dialog>` opened with `showModal()`, never a hand-rolled `<div>` overlay — the native element supplies focus trapping, `Esc`-to-close, and an inert background for free, exactly what a keyboard or screen-reader user depends on and a hand-rolled overlay silently drops. Open/closed state lives in the URL, not React state: the page renders the dialog when its triggering query param is present and closing navigates back to the same URL without it, so the back button closes it and the view is linkable (see the owner calendar's day dialog, `src/components/dashboard/day-bookings-dialog.tsx`, keyed off `?day=`). Needs `aria-labelledby` pointing at the dialog's own heading and a real `<button>` (not a styled span) to close. Must also carry `m-auto`: Tailwind v4's preflight sets `margin: 0` on `*, ::before, ::after, ::backdrop`, and an author-origin rule beats the UA stylesheet's own `dialog { margin: auto }` — which, paired with `dialog:modal { inset: 0 }`, is the only thing that centers a `showModal()` dialog. Without `m-auto` the dialog renders pinned to the top-left corner (`getBoundingClientRect()` reads `{top: 0, left: 0}`) instead of centered. Backdrop `rgba(6,20,13,.45)`; panel uses the card base recipe above in full, shadow included — `--panel` fill, 20px radius, no border, shadow `--shadow-sm`.

  **Documented exception — the venue page's photo gallery lightbox** (`src/components/branch/photo-gallery.tsx`): three of this entry's rules bend on purpose here, each named so the next reader finds the reasoning instead of assuming drift. (1) **Open/closed state lives in React (`openIndex`), not the URL.** The URL convention above exists for state worth bookmarking or reaching via the back button (an owner's day dialog); a lightbox index is ephemeral browsing view state on top of a page whose own address already carries `?date=`, and nobody needs to link to "photo 3 of 6" mid-browse, so a second query param wasn't worth stacking on. (2) **`aria-label` rather than `aria-labelledby`.** A lightbox has no heading — it's a photo and some controls — so there is no element for `aria-labelledby` to point at; naming the dialog directly with `aria-label` is the honest choice here, not a shortcut. (3) **The panel is `bg-[var(--ink)]` with no shadow**, not this entry's `--panel` fill/shadow. A dark surround is correct for a photo viewer — it lets the photo itself read as the content instead of competing with a white card edge — so this is a second, deliberate modal skin scoped to the lightbox, not a gap in the rule above. Any future modal that isn't a photo viewer should still default to the light `--panel` skin; the dark skin doesn't generalize past this one call site.
...
- The admin court queue card (`src/app/admin/page.tsx`) is not a single
  ...
  focus, the same reasoning that keeps a panel un-lifted. It also uses `p-5`
  body padding (`max-[560px]:p-4`) instead of the standard `px-5 pt-[18px]
  ...
  plain title+meta card. As of a 2026-09-08 density pass this card is also
  deliberately shorter than every other entity card, since the moderation
  queue is scanned rather than read top to bottom: the cover is
  `aspect-[16/7]` rather than the standard `16/10`; the title steps down from
  the standard card `text-lg`/18px to `text-[16px]` tracking-[-0.01em] (the
  same smaller card-title size already used by `src/app/bookings/page.tsx`
  and `src/app/admin/refunds/page.tsx`); and the price line steps down from
  branch-card.tsx's 15px to `text-[13.5px]` — still its own bolder line above
  the fact list, just one size smaller. Padding was cut a step in the same
  pass, from the card's original `p-6`/`p-5` pair, once the shorter cover and
  smaller type freed up enough room that `p-6` no longer earned its keep.
  The grid's gap tightened from `gap-4` to `gap-3` alongside it, and the
  body's internal vertical rhythm (the price line, fact list, and
  rejection-reason spacing, and the footer action row) moved from `mt-3`/
  `mt-4`/`pt-4` to `mt-2.5`/`mt-3`/`pt-3` throughout. The grid itself stepped
  from two columns to three in the same pass, collapsing to `grid-cols-2` at
  980px and `grid-cols-1` at 560px — both of this doc's Breakpoints entry and
  no others; see the comment above the `<ul>` in `src/app/admin/page.tsx` for
  why three now fits.
...
- This dash is for a *rendered* range only — text the page prints, like `formatHourRange`'s output. It's the wrong choice between two live controls: a dash sitting between two `<select>`s reads as a hyphen inside a value rather than as a connector joining them. A from/to pair of dropdowns (the search bars' Time-range field, see Controls above) spells it out with the word "to" instead.
